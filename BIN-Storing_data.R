...
# works:  id, person$id, book$id #The 'works' table is a join table between person and books.
...
#factors are categorical variables. Both numeric and character
#variables can be converted to factors.
...
#identifies the book(s) corresponding to each author so that
#they can be printed together within the same for loop cycle.
...
x <- jsonlite::fromJSON("./data/MBP1_SACCE.json") #fromJSON() converts the JSON syntax to R syntax.
str(x)
...
# Try:
#dbSanitizeSequence() ignores all non-nucleotide letters,
#ignores all whitespace, and converts lower case to upper case.
...
#dbInit() creates 4 lists, namely 'protein', 'taxonomy',
...
source("./scripts/ABC-createRefDB.R") #If fromJSON() in "./scripts/ABC-createRefDB.R" with jsonlite::fromJSON() returns error that fromJSON() isn't recognized, run library("jsonlite")
...
all(myDB$protein$taxonomyID %in% myDB$taxonomy$ID) #x %in% X returns TRUE if element x is in Vector X
...
sel <- myDB$taxonomy$ID == x #In order to get species name from protein name, need to cross-reference from protein list to taxonomy list
...
#
# - Click "BLAST". #The closest NP_010227 homologue in Gaeumannomyces tritici (MYSPE) is XP_009229257.1
...
# - Next make a copy of the file "./data/MBP1_SACCE.json" in your project #For the sequence in my "MBP1_GAETR.json" file, I copy-pasted from the FASTA file
